...
export default function Page() {
  ...
  async function speak(text: string) { try { const response = await fetch('/api/voice', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text }) }); if (response.ok) { const audio = new Audio(URL.createObjectURL(await response.blob())); audio.onended = () => setMode('idle'); await audio.play(); return } } catch { /* Fall back to browser speech synthesis. */ } if ('speechSynthesis' in window) { window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); utterance.rate = 0.92; utterance.pitch = 0.78; utterance.onend = () => setMode('idle'); window.speechSynthesis.speak(utterance) } else setMode('idle') }
